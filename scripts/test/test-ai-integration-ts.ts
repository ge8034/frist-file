/**
 * AI系统集成测试脚本 (TypeScript版本)
 *
 * 测试AI系统与游戏规则服务的集成
 */

console.log('=== AI系统集成测试 (TypeScript) ===\n')

// 模拟游戏规则服务
const mockGameRuleService = {
  validatePlay: (playerId: string, cards: any[], currentPattern?: any, gameSession?: any) => ({
    valid: true,
    message: '模拟验证通过'
  })
}
...
async function runTest() {
  try {
    // 导入AI模块
    const aiModule = await import('../../lib/features/game/ai/index.ts')
    ...
    aiPlayer.setHandCards(handCards as any)
    console.log('✅ 手牌设置成功')

    // 更新游戏状态
    aiPlayer.updateGameState(mockGameSession as any)
    console.log('✅ 游戏状态更新成功')

    // 获取可能的出牌选项
    const possiblePlays = aiPlayer.getPossiblePlays(mockGameSession as any)
    console.log(`✅ 获取到 ${possiblePlays.length} 个出牌选项`)

    if (possiblePlays.length > 0) {
      console.log('出牌选项:')
      possiblePlays.forEach((play, index) => {
        console.log(`  ${index + 1}. ${play.choice}: ${play.cards?.length || 0}张牌，评分: ${play.score}`)
      })
    }
